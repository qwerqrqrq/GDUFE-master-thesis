# 更新日志

## v0.5.6

* 添加了子图引用的相关设置
* 现在英文摘要第一段有空格
* 修正了封面底部日期错误设置为答辩日期的问题
* 添加了修改数学公式字体支持
* 修改了页眉边距，现在与word近似
* 修改了引用子图的相关内容
* 回退了list设置，现在参考文献更加密集了
* 更改了单双页的相关设置，现在使用中山大学论文模板的实现方式

## v0.5.5

* 增加了一个纯白图片用于替换签名图片
* 增加了盲审去除代码生成封面、扉页、声明信息的代码
* 修改签名图片文件名
* 修复了参考文献表字号不对的问题
* 修复了使用代码生成的封面论文标题不是加粗的问题
* 修复了代码生成封面的个人信息内容不是宋体的问题
* 增加了定理编号、列表使用说明
* 尝试修复了图片与上下文不正常空白的问题

## v0.5.4

### 增加了

* 增加了定理编号格式设置

### 修改了

* 定理环境包从 ntheorem 转成 amsthm, thmtools

## v0.5.3

### 修复了

* 修复了图表标题数学公式没有加粗的问题

* 修复了图表字号不对的问题

### 修改了

* 修改了 Overleaf 指引图片

### 增加了

* 增加了允许数学公式跨页显示

### 删除了

* 删除了参考文献中的 url 和 doi

## v0.5.2

### 修复了

* 修复了 LaTeX Font: Font shape `U/rsfs/m/n' in size <10.53937> not available (Font)size <10.95> substituted. 警告

### 修改了

* 修改了宏包说明

## v0.5.1

### 添加了

* 添加了 TexStudio 的使用说明

## v0.5.0

### 添加了

* 现在能支持开题报告

## v0.4.4

## 修改了

* 修改了图标标题的间距 0pt $\rightarrow$ 5pt

* 修改了一些使用说明

## 删除了

* 删除了摘要的多余说明

## v0.4.3

### 添加了

* 现在支持长表格

* 增加了部分使用说明

### 修复了

* 修复了子图不能正确引用的问题

## v0.4.2

### 添加了

* 开题报告的一些内容

* 添加了彩色校徽

* 添加了一部分使用说明

### 修改了

* 现在lstlisting 默认为彩色，forprint会影响lstlisting

* 修改了代码环境的样式

* 修正了生成专硕的封面、标题页错误的问题

## v0.4.1

### 修改了

* 修改了生成封面、标题、声明页的相关代码
* 修改了摘要页格式的实现方法
* 修改了控制章节条款格式的方法

### 修正了

* proof环境

### 新增了

* 新增一些图像文件

### 删除了

* 删除了solution环境

## v0.4.0

### 调整了

* 调整了模板文件章节内容
* 调整了引用包的顺序

### 修复了

* 修复了右侧页眉不显示论文标题的问题

## v0.3.2

### 修复了

* 修正了英文摘要行距, 正文行距不对的问题
* 修正了目录页格式不对的问题

### 调整了

* 调整\zihao \heiti等命令，使得后面自动设置20pt行距

### 新增了

* 使用\the\baselineskip 显示了行距

## v0.3.1

### 新增问题

* 页眉页脚页边距不对

### 修复了

* 修改了行距设置

### 调整了

* 现在章节在 includefile/chapter*.tex 中

## v0.3.0

删库重新上传

### 增加了

* 电子书签序号

### 修复了

* 章节条款格式不正确的问题
* 现在有正常的和上标的参考引用方式

### 调整了

* tex文件中参考文献引用的说明
* bib文件, thebibliography表的内容


## v0.2.0

### 修复了

* 字体样式叠加不正确的问题

##  v0.1.0
**发布的第一个版本**